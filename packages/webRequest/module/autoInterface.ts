...
import { json2url, isArray, isJSON, getToken } from 'utils';
...
interface IHttpFn<T = any> { (): Promise<T> };
...
class AutoInterface {
    ...
    constructor(list: jsonObject, platformKey: string) {
        ...
        Object.entries(list).forEach((item, index) => {
            ...
            const filePath: string = `@/api/${key}.api.ts`;
            if (isJSON(value)) {// json
                const v: jsonObject = value;
                for (let k in v) { v[k]['key'] = k; };
                this.mountMethods(key, Object.entries(value).map(([, v], index) => v), `${filePath}>>${key}`);
            } else if (isArray(value)) {// 数组
                this.mountMethods(key, value, `${filePath}>>${key}`);
            } else {
                notification.error({ message: '接口解析错误!', description: `${filePath},文件解析失败,请参考标准格式！` });
            };
        })
    };
    ...
    mountedMethod(item: jsonObject, path: string) {
        ...
        return (args: jsonObject = {}) => {
            ...
            let verifyed: verifyedParam = { params: {}, config: {} },
                unPassed: jsonObject[] = new Array,
                required: jsonObject[] = new Array;

            const { key, url, type, params = [] } = item;
            for (let i = 0, j = params.length; i < j; i++) {
                // 校验主参数
                const { key: k, targetKey: tk, valueAttrs = {}, location, cn, description: desc }: jsonObject = params[i];
                // 校验key 属性
                ...
                if (!args.hasOwnProperty(k) && !mandatorykey && (location && location != 'header')) {
                    ...
                    if (location === 'header') {
                        if (rtk.toLocaleLowerCase() === 'Authorization'.toLocaleLowerCase()) rdv = `Bearer ${rdv || getToken(_this.platformKey)}`;
            ...
            if (required.length > 0) return _this.errorMethod(`文件路径:${path}；${required.map(i => {
                return `${i.key}(${i.cn || '无描述'}) 为必填项`
            }).join("、")}，请检查！！！`);
            if (unPassed.length > 0) return _this.errorMethod(`文件路径:${path}；缺少字段： ${unPassed.map(i => {
            ...
            if (service.hasOwnProperty(xhrType)) return service[xhrType](url, verifyed.params, verifyed.config);
     ...
     * error 
     */
    errorMethod(msg: string = "['validation failed']:Parameter validation failed, please check the parameters!") {
        notification.error({ message: '参数解析错误!', duration: null, description: `${msg}`, style: { color: '#ff4d4f' } });
