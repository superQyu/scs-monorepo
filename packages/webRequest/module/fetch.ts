 ...
 * Desc: 对fetch的封装
 */
import { json2url, isArray } from 'utils';
...
const request = (url: string, setting: any) => {
    ...
    return new Promise((resolve, reject) => {
        fetch(url, opts).then(async res => {
            let resObject = dataType === 'text' ? await res.text() : dataType === 'blob' ? await res.blob() : await res.json();

            // if (interceptor.response) interceptor.response(resObject);

            netStatus(requested || {}, resObject);

            !res.ok ? reject(resObject) : resolve(resObject[requested.dataKeyName || 'data'] || resObject);
        }).catch(e => {
...
const paramsApart = (obj: object = {}, os: stringKey = {}) => {
    ...
    let headers: stringKey = {};
    let excepted: stringKey = {};
...
export const fetchRequest = {
    async get(url: string, params: Object = {}, config = {}) { // get
        try {

            let res = await request(json2url({ url, params }), {
                method: 'GET',
                ...paramsApart(config, {
                    headers: {}
                })
    ...
    async post(url: string, params = {}, config = {}) {// post
        try {
    ...
    async delete(url: string, params: Object = {}, config = {}) { // delete
        try {
